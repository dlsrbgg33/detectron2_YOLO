...
from detectron2.data import (
    ...
    build_detection_train_loader_vda,
)
...
class DefaultTrainer(SimpleTrainer):
    ...
    def __init__(self, cfg):
        ...
        # data_loader_list = []
        if not cfg.DATASETS.TASK_VIDEO_DA:
            data_loader = self.build_train_loader(cfg)
            data_loader_list = [data_loader]
        else:
            data_loader, data_loader_ct = self.build_train_loader_vda(cfg)
            data_loader_list = [data_loader, data_loader_ct]
        # import pdb
        # pdb.set_trace()
        # data_loader = [data_loader, data_loader_ct]
        # For training, wrap with DDP. But don't need this for inference.
        if comm.get_world_size() > 1:
            model = DistributedDataParallel(
                model, device_ids=[comm.get_local_rank()], broadcast_buffers=False
            , find_unused_parameters=True)
        super().__init__(model, data_loader_list, optimizer)
    ...
    def build_hooks(self):
        ...
        if not self.cfg.DATASETS.TASK_VIDEO_DA:
            ret = [
            ...
            ret = [
                ...
                hooks.PreciseBN(
                    ...
                    self.model,
                    # Build a new data loader to not affect training
                    self.build_train_loader_vda(cfg),
                    cfg.TEST.PRECISE_BN.NUM_ITER,
                )
                if cfg.TEST.PRECISE_BN.ENABLED and get_bn_modules(self.model)
                else None,
    ...
    @classmethod
    def build_train_loader_vda(cls, cfg):
        """
        Returns:
            iterable
        ...
        Overwrite it if you'd like a different data loader.
        """
        return build_detection_train_loader_vda(cfg)
