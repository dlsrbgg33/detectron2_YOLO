...
try:
    ...
    from detectron2.layers import ModulatedDeformConv as DCN
except ImportError:
    raise ImportError('Failed to import DCNv2 module.')
from . import DeformConvWithOffset
# from mmcv.cnn import xavier_init, kaiming_init
# from detectron2.utils import xavier_init
# from detectron2.utils import kaiming_init
import pdb
...
class PCD_Align_4(nn.Module):
    ...
    def __init__(self, nf=64, groups=8):
        ...
        # L4: level 4, 1/8 spatial size
        self.L4_offset_conv1 = nn.Conv2d(nf * 2, nf, 3, 1, 1, bias=True)  # concat for diff
        self.L4_offset_conv2 = nn.Conv2d(nf, nf, 3, 1, 1, bias=True)
        self.L4_dcnpack = DCN(nf, nf, 3, stride=1, padding=1, dilation=1, deformable_groups=groups,
                              extra_offset_mask=True)
        # L3: level 3, 1/4 spatial size
        self.L3_offset_conv1 = nn.Conv2d(nf * 2, nf, 3, 1, 1, bias=True)  # concat for diff
        self.L3_offset_conv2 = nn.Conv2d(nf * 2, nf, 3, 1, 1, bias=True)  # concat for offset
        self.L3_offset_conv3 = nn.Conv2d(nf, nf, 3, 1, 1, bias=True)
    ...
    def forward(self, nbr_fea_l, ref_fea_l):
        ...
        nbr_fea_l, ref_fea_l: [L1, L2, L3, L4], each with [B,C,H,W] features
        '''
        # L4
        L4_offset = torch.cat([nbr_fea_l[3], ref_fea_l[3]], dim=1)
        L4_offset = self.lrelu(self.L4_offset_conv1(L4_offset))
        L4_offset = self.lrelu(self.L4_offset_conv2(L4_offset))
        L4_fea = self.lrelu(self.L4_dcnpack([nbr_fea_l[3], L4_offset]))
        # L3
        L3_offset = torch.cat([nbr_fea_l[2], ref_fea_l[2]], dim=1)
        L3_offset = self.lrelu(self.L3_offset_conv1(L3_offset))
        L4_offset = F.interpolate(L4_offset, scale_factor=2, mode='bilinear', align_corners=False)
        L3_offset = self.lrelu(self.L3_offset_conv2(torch.cat([L3_offset, L4_offset * 2], dim=1)))
        L3_offset = self.lrelu(self.L3_offset_conv3(L3_offset))
        L3_fea = self.L3_dcnpack([nbr_fea_l[2], L3_offset])
        L4_fea = F.interpolate(L4_fea, scale_factor=2, mode='bilinear', align_corners=False)
        L3_fea = self.lrelu(self.L3_fea_conv(torch.cat([L3_fea, L4_fea], dim=1)))
        # L2
...
class PCD_Align(nn.Module):
    ...
    def forward(self, nbr_fea_l, ref_fea_l, bsf=None):
        '''align other neighboring frames to the reference frame in the feature level
...
class DeformAlign(nn.Module):
    ''' Alignment module using Cascading and Deformable convolution
    with single level.
    '''

    def __init__(self, nf=64, groups=8):
        super(DeformAlign, self).__init__()

        self.theta_conv1 = nn.Conv2d(nf*2, nf, 3,1,1, bias=True) # concat for theta
        self.theta_conv2 = nn.Conv2d(nf, nf, 3,1,1, bias=True)
        self.dcnpack = DCN(nf, nf, 3, stride=1, padding=1, dilation=1,
                           deformable_groups=groups, extra_offset_mask=True)
        # Cascading DCN
        self.cas_theta_conv1 = nn.Conv2d(nf*2, nf, 3,1,1, bias=True)
        self.cas_theta_conv2 = nn.Conv2d(nf, nf, 3,1,1, bias=True)
        self.cas_dcnpack = DCN(nf, nf, 3, stride=1, padding=1, dilation=1,
                               deformable_groups=groups, extra_offset_mask=True)

        self.lrelu = nn.LeakyReLU(negative_slope=0.1, inplace=True)

    def forward(self, nbr_bsf, ref_bsf):
        '''align other neighboring frames to the reference frame in the feature level
        nbr_bsf, ref_bsf: each with [B,C,H,W] features
        '''
        # Coarse offset + align
        theta = torch.cat([nbr_bsf, ref_bsf], dim=1)
        theta = self.lrelu(self.theta_conv1(theta))
        theta = self.lrelu(self.theta_conv2(theta))
        coarse_bsf = self.lrelu(self.dcnpack([nbr_bsf, theta]))
        # Casecade
        theta1 = torch.cat([coarse_bsf, ref_bsf], dim=1)
        theta1 = self.lrelu(self.cas_theta_conv1(theta1))
        theta1 = self.lrelu(self.cas_theta_conv2(theta1))
        fine_bsf = self.lrelu(self.cas_dcnpack([coarse_bsf, theta1]))
...
#         att = F.interpolate(att, scale_factor=2, mode='bilinear', align_corners=False)
#         att = self.sAtt_3(att)
#         att_add = self.sAtt_add_1(self.lrelu(att))
#         att = torch.sigmoid(att)
...
class TA_Fusion(nn.Module):
    ...
    def __init__(self, nf=64, nframes=3, center=1):
        ...
        # fusion conv: using 3x3 to save parameters and computation
        self.fea_fusion = nn.Conv2d(nframes * nf, nf, 3, 1, 1, bias=True)
        # self.lrelu = nn.LeakyReLU(negative_slope=0.1, inplace=True)
        # self.score = nn.Conv2d(nf,nf,1)
...
class ST_Aggregator(nn.Module):
    def __init__(self, in_ch, T):
        super(ST_Aggregator, self).__init__()
        ### spatio-temporal aggregation
        self.stAgg =  nn.Conv3d(in_ch, in_ch, kernel_size=(T,3,3), stride=(1,1,1), padding=(0,1,1), bias=False)
