...
def build_transform_gen(cfg, is_train):
    ...
    Returns:
        list[TransformGen]
    """
    is_crop = cfg.INPUT.CROP_ON
    if is_train:
        min_size = cfg.INPUT.MIN_SIZE_TRAIN
        max_size = cfg.INPUT.MAX_SIZE_TRAIN
        sample_style = cfg.INPUT.MIN_SIZE_TRAIN_SAMPLING
        crop_size = cfg.INPUT.CROP_SIZE
        crop_type = cfg.INPUT.CROP_TYPE
    else:
    ...
    logger = logging.getLogger(__name__)
    tfm_gens = []
    if cfg.INPUT.RATIO_RESIZE:
        tfm_gens.append(T.ResizeShortestEdge(min_size, max_size, sample_style))
        if is_train and is_crop:
            tfm_gens.append(T.RandomCrop(crop_type, crop_size))
    else:
        tfm_gens.append(T.Resize((min_size, max_size)))
    if is_train:
